// content.js
(function () {
  const t = (k) => (typeof chrome !== 'undefined' && chrome.i18n && chrome.i18n.getMessage(k)) || k;
  ...
  function mountPanel(slug) {
    ...
    panel.innerHTML = `
      ...
      <div class="pb-row pb-dir">${t('direction')}:
        <button data-dir="YES" class="active">YES</button>
        <button data-dir="NO">NO</button></div>
      <div class="pb-row">${t('buyPerBin')} <input type="number" id="pb-shares" value="100" min="1"> ${t('sharesWord')}</div>
      <div id="pb-bins">${t('loading')}</div>
      <div class="pb-total" id="pb-total"></div>
      <button class="pb-submit" id="pb-submit">${t('submit')}</button>`;
    ...
    async function recompute() {
      ...
      for (let i = 0; i < state.bins.length; i++) {
        ...
        catch { if (gen !== _gen) return; span.textContent = t('loadFailed'); span.className = 'pb-leg pb-warn'; ok = false; continue; }
        if (gen !== _gen) return;
        const r = calcLegCost(book, state.shares);
        if (!r.enough) { span.textContent = t('insufficientDepth'); span.className = 'pb-leg pb-warn'; ok = false; continue; }
      ...
      if (!avgPrices.length) { el.textContent = `${n} ${t('legsWord')} · 0 ${t('sharesWord')}`; return; }
      const s = summarize(avgPrices, state.shares);
      const cls = s.profitable ? 'pb-profit' : 'pb-loss';
      const verdict = s.profitable ? t('profitable') : t('willLose');
      el.innerHTML =
        `${n} ${t('legsWord')} · ${totalShares} ${t('sharesWord')}<br>` +
        `${t('combinedCost')}: <span class="${cls}">$${s.combined.toFixed(3)} · ${verdict}</span><br>` +
        `${t('totalSpend')}: $${s.totalSpend.toFixed(2)}` +
        (ok ? '' : `<br><span class="pb-warn">${t('partialDepth')}</span>`);
    }
  ...
  // SPA 路由：hook history + popstate，站内导航到新 event 页时重挂面板
  const _ps = history.pushState; history.pushState = function () { _ps.apply(this, arguments); onRouteChange(); };
  const _rs = history.replaceState; history.replaceState = function () { _rs.apply(this, arguments); onRouteChange(); };
